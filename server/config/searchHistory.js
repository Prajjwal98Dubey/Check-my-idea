...
const addSearchedQueryToRedis = async (req, res) => {
    ...
    if (userSearchHistory) {
        ...
        let flag = 1;
        for (let i = 0; i < parsedUserSearchHistoryList.length; i++) {
            if (parsedUserSearchHistoryList[i]._id === searchQuery) flag = 0
        }
        if (flag) parsedUserSearchHistoryList.push(product)
