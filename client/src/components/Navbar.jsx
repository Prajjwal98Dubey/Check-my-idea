    ...
    <ul>
        {
            !isLoading &&
            <>
            {!userHistoryLoading && userHistoryResult && userHistoryResult.map((item,index)=>(
                <>
                ...
                {searchResult.products.map((search, index) => (
                    <>
                        <Link key={index} to={"/idea?id=" + search._id}><li className='p-1 font-semibold hover:bg-gray-400 hover:cursor-pointer m-1 rounded-lg' onClick={() => handleUserSearchHistory(search._id)}>{search.name}</li></Link></>
                ))}
            </>
        }
